...
const SoftKeyboard: React.FC<{ onKeyPress: (key: string) => void }> = ({ onKeyPress }) => (
  <div className="grid gap-1.5 p-4 bg-neutral-900/90 rounded-2xl border border-white/5 backdrop-blur-xl shadow-inner">
    {KEYBOARD_LAYOUT.map((row, rowIndex) => (
      <div key={rowIndex} className="flex gap-1.5 justify-center">
        {row.map((key, keyIndex) => {
          if (key === '') return <div key={keyIndex} className="w-10 h-10" />;
          ...
          return (
            <button key={keyIndex} onClick={() => onKeyPress(displayKey[0])}
              className={`w-10 h-10 flex flex-col items-center justify-center ${isSpecial ? 'bg-neutral-800 text-neutral-400 hover:bg-neutral-750' : 'bg-neutral-700 text-white hover:bg-neutral-600'} active:scale-90 active:brightness-75 text-[10px] font-black rounded-xl shadow-lg transition-all border-b-4 border-black/40`}
            >
