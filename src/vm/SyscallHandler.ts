...
export class SyscallHandler {
    ...
    public async handle(op: number): Promise<number | void> {
        ...
        switch (op) {
            ...
            case SystemOp.printf: {
                const count = vm.pop();
                const fmtHandle = vm.stk[vm.sp - count];
                const formatBytes = vm.getStringBytes(fmtHandle);
                if (formatBytes) {
                    const str = this.formatVariadicString(formatBytes, count - 1, vm.sp - count + 1);
            ...
            case SystemOp.sprintf: {
                const count = vm.pop();
                const fmtHandle = vm.stk[vm.sp - count];
                const destAddr = vm.resolveAddress(vm.stk[vm.sp - count - 1]);
                const formatBytes = vm.getStringBytes(fmtHandle);
                if (formatBytes) {
                    const str = this.formatVariadicString(formatBytes, count - 1, vm.sp - count + 1);
            ...
            case SystemOp.Delay: await new Promise(r => setTimeout(r, vm.pop())); return;
            ...
            case SystemOp.Exit: vm.pop(); vm.running = false; return;
            case SystemOp.ClearScreen: vm.memory.fill(0, BUF_OFFSET, BUF_OFFSET + 1600); return;
            case SystemOp.abs: result = Math.abs(vm.pop()); break;
            case SystemOp.rand: result = (Math.random() * 0x8000) | 0; break;
            case SystemOp.srand: vm.pop(); return;
            ...
            case SystemOp.fclose: vm.vfs.closeFile(vm.pop()); return;
            ...
            case SystemOp.fread: {
                ...
                const toRead = Math.min(totalBytes, h.data.length - h.pos);
                if (toRead > 0) {
                    vm.memory.set(h.data.slice(h.pos, h.pos + toRead), bufAddr);
                    h.pos += toRead;
                }
                result = (toRead / size) | 0;
                break;
            }
            case SystemOp.fwrite: {
                ...
                const bufAddr = vm.resolveAddress(vm.pop());
                const totalBytes = count * size;
                const data = vm.memory.slice(bufAddr, bufAddr + totalBytes);
                const h = vm.vfs.getHandle(fp);
                if (h) {
            ...
            case SystemOp.Math: {
                const sub = vm.pop();
                switch (sub) {
                    case MathOp.itof: return (vm.pop()); // Return as int bits representing float
                    case MathOp.ftoi: result = (vm.popFloat() | 0); break;
                    case MathOp.fadd: {
                        const b = vm.popFloat(); const a = vm.popFloat();
                        const buf = new ArrayBuffer(4);
                        new Float32Array(buf)[0] = a + b;
                    ...
                    case MathOp.sin: {
                        const val = vm.popFloat();
                        const buf = new ArrayBuffer(4);
                        new Float32Array(buf)[0] = Math.sin(val);
                        return new Int32Array(buf)[0];
                    }
                    case MathOp.cos: {
                        const val = vm.popFloat();
                        const buf = new ArrayBuffer(4);
                        new Float32Array(buf)[0] = Math.cos(val);
                        return new Int32Array(buf)[0];
                    }
                    case MathOp.tan: {
                        const val = vm.popFloat();
                        const buf = new ArrayBuffer(4);
                        new Float32Array(buf)[0] = Math.tan(val);
                        return new Int32Array(buf)[0];
                    }
                    case MathOp.sqrt: {
                        const val = vm.popFloat();
                        const buf = new ArrayBuffer(4);
                        new Float32Array(buf)[0] = Math.sqrt(val);
                        return new Int32Array(buf)[0];
                    }
                    case MathOp.fabs: {
                        const val = vm.popFloat();
                        const buf = new ArrayBuffer(4);
                        new Float32Array(buf)[0] = Math.abs(val);
                        return new Int32Array(buf)[0];
                    }
                    default: return;
                }
