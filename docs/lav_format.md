...
| `0x04` | 1 | reserved04 | 官方样本中固定为 `0x00` |
| `0x05` | 1 | strMask | 字符串 XOR 掩码（`0x00` = 无掩码；`PUSH_STR` 读取每字节后 XOR 该值直到结果为 0） |
| `0x06` | 2 | arrayInitSpace | `#loadall` 段大小（小端序 u16，官方样本通常为 `0x0000`） |
| `0x08` | 3 | entryPointField | 官方样本中固定为 `0x000000`；运行时入口按偏移 `0x10` 起始 |
| `0x0B` | 5 | reserved0B0F | 需要按原样保留；部分官方样本在这些字节中带有签名字样，不能统一重写为 0 |

> **兼容性说明**：LavStudio 当前以仓库内官方 `.lav` 样本为事实标准。样本表明 `0x08-0x0A` 需要保持为 0，而 `0x0B-0x0F` 不是稳定的全零填充字段；解析和重组时必须保留原始头部字节。
