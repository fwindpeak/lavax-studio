...
export class LavaXVM {
  ...
  private async loadVFSFromStorage() {
    const saved = localStorage.getItem('lavax_vfs');
    if (saved) {
      try {
  ...
  private saveVFSToStorage() {
    ...
    localStorage.setItem('lavax_vfs', JSON.stringify(obj));
  }
  ...
  load(lav: Uint8Array) {
    ...
    for (let i = 0; i < stringsTail.length; i++) {
      if (stringsTail[i] === 0) {
        this.stringPool.push(new Uint8Array(currentStr));
        currentStr = [];
      } else {
  ...
  async run() {
    ...
    try {
      while (this.running && this.ip < this.code.length) {
        await this.step();
        if (this.ip % 500 === 0) await new Promise(r => setTimeout(r, 0));
      }
    } catch (e: any) { 
      this.onLog(`VM Error: ${e.message}`); 
    }
    this.running = false;
    this.onFinished();
  }
  ...
  pushKey(key: string) { 
    const keyMap: Record<string, number> = { '↵': 13, 'ESC': 27, '↑': 20, '↓': 21, '←': 22, '→': 23 };
  ...
  private flushScreen() {
    ...
    for (let i = 0; i < SCREEN_WIDTH * SCREEN_HEIGHT; i++) {
      ...
      const idx = i * 4;
      const c = pixel ? [30, 40, 30] : [148, 161, 135];
  ...
  private drawText(x: number, y: number, obj: any) {
    ...
    while (i < bytes.length) {
      ...
      if (b1 < 0x80) {
        ...
        const b2 = bytes[i+1];
        if (b2) { this.drawChinese(curX, y, b1, b2, this.currentFontSize); curX += this.currentFontSize; i+=2; }
        else i++;
      }
    }
  }
  ...
  private drawChar(x: number, y: number, code: number, size: number) {
    ...
    for (let r = 0; r < size; r++) {
      ...
      for (let c = 0; c < width; c++) if ((byte >> (7 - c)) & 1) this.setPixel(x + c, y + r, 1);
    }
  }
  ...
  private drawChinese(x: number, y: number, b1: number, b2: number, size: number) {
    ...
    const charSize = size === 16 ? 32 : 24;
    const offset = base + (rIdx * 94 + cIdx) * charSize;
    for (let r = 0; r < size; r++) {
      const bL = this.fontData![offset + r * 2], bR = this.fontData![offset + r * 2 + 1];
      for (let b = 0; b < 8; b++) if ((bL >> (7-b)) & 1) this.setPixel(x+b, y+r, 1);
      for (let b = 0; b < size-8; b++) if ((bR >> (7-b)) & 1) this.setPixel(x+8+b, y+r, 1);
    }
  }

  private drawBox(x: number, y: number, w: number, h: number) {
    for(let i=x; i<x+w; i++) { this.setPixel(i, y, 1); this.setPixel(i, y+h-1, 1); }
    for(let i=y; i<y+h; i++) { this.setPixel(x, i, 1); this.setPixel(x+w-1, i, 1); }
  }
